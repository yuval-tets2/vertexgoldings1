/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { AdmissionFormCreateNestedManyWithoutAdmissionBatchesInput } from "./AdmissionFormCreateNestedManyWithoutAdmissionBatchesInput";
import { ValidateNested, IsOptional, IsDate, IsString } from "class-validator";
import { Type } from "class-transformer";
import { CourseCreateNestedManyWithoutAdmissionBatchesInput } from "./CourseCreateNestedManyWithoutAdmissionBatchesInput";

@InputType()
class AdmissionBatchCreateInput {
  @ApiProperty({
    required: false,
    type: () => AdmissionFormCreateNestedManyWithoutAdmissionBatchesInput,
  })
  @ValidateNested()
  @Type(() => AdmissionFormCreateNestedManyWithoutAdmissionBatchesInput)
  @IsOptional()
  @Field(() => AdmissionFormCreateNestedManyWithoutAdmissionBatchesInput, {
    nullable: true,
  })
  admissionForm?: AdmissionFormCreateNestedManyWithoutAdmissionBatchesInput;

  @ApiProperty({
    required: false,
    type: () => CourseCreateNestedManyWithoutAdmissionBatchesInput,
  })
  @ValidateNested()
  @Type(() => CourseCreateNestedManyWithoutAdmissionBatchesInput)
  @IsOptional()
  @Field(() => CourseCreateNestedManyWithoutAdmissionBatchesInput, {
    nullable: true,
  })
  courses?: CourseCreateNestedManyWithoutAdmissionBatchesInput;

  @ApiProperty({
    required: true,
  })
  @IsDate()
  @Type(() => Date)
  @Field(() => Date)
  endDate!: Date;

  @ApiProperty({
    required: true,
    type: String,
  })
  @IsString()
  @Field(() => String)
  name!: string;

  @ApiProperty({
    required: true,
  })
  @IsDate()
  @Type(() => Date)
  @Field(() => Date)
  startDate!: Date;
}

export { AdmissionBatchCreateInput as AdmissionBatchCreateInput };
